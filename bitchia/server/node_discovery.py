...
from bitchia.protocols import full_node_protocol, introducer_protocol
from bitchia.protocols.protocol_message_types import ProtocolMessageTypes
from bitchia.server.address_manager import AddressManager, ExtendedPeerInfo
from bitchia.server.address_manager_store import AddressManagerStore
from bitchia.server.outbound_message import NodeType, make_msg
from bitchia.server.server import BitChiaServer
from bitchia.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: BitChiaServer = server
    ...
    async def on_connect(self, peer: ws.WSBitChiaConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSBitChiaConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSBitChiaConnection):
