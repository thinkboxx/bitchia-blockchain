...
import bitchia.server.ws_connection as ws
from bitchia.consensus.block_creation import create_unfinished_block
from bitchia.consensus.block_record import BlockRecord
from bitchia.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from bitchia.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from bitchia.full_node.full_node import FullNode
from bitchia.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from bitchia.full_node.signage_point import SignagePoint
from bitchia.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from bitchia.protocols.full_node_protocol import RejectBlock, RejectBlocks
from bitchia.protocols.protocol_message_types import ProtocolMessageTypes
from bitchia.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from bitchia.server.outbound_message import Message, make_msg
from bitchia.types.blockchain_format.coin import Coin, hash_coin_list
from bitchia.types.blockchain_format.pool_target import PoolTarget
...
from bitchia.types.mempool_item import MempoolItem
from bitchia.types.peer_info import PeerInfo
from bitchia.types.unfinished_block import UnfinishedBlock
from bitchia.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from bitchia.util.generator_tools import get_block_header
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSBitChiaConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSBitChiaConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSBitChiaConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSBitChiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    @peer_required
    @api_request
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSBitChiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSBitChiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSBitChiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
