...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the bitchia python virtual environment."
...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("bitchiavdf").location)' 2>/dev/null)/vdf_client
CHIAVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("bitchiavdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing bitchiavdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary bitchiavdf "$CHIAVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary bitchiavdf "$CHIAVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing bitchiavdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing bitchiavdf requirement boost for MacOS."
		brew install boost
		echo "Installing bitchiavdf from source."
