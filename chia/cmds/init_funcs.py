...
from bitchia.consensus.coinbase import create_puzzlehash_for_pk
from bitchia.ssl.create_ssl import generate_ca_signed_cert, get_bitchia_ca_crt_key, make_ca_cert
from bitchia.util.bech32m import encode_puzzle_hash
from bitchia.util.config import (
    create_default_bitchia_config,
    initial_config_file,
    load_config,
    save_config,
...
from bitchia.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'bitchia keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    bitchia_ca_crt, bitchia_ca_key = get_bitchia_ca_crt_key()
    bitchia_ca_crt_path = ca_dir / "bitchia_ca.crt"
    bitchia_ca_key_path = ca_dir / "bitchia_ca.key"
    bitchia_ca_crt_path.write_bytes(bitchia_ca_crt)
    bitchia_ca_key_path.write_bytes(bitchia_ca_key)
    ...
    bitchia_ca_crt, bitchia_ca_key = get_bitchia_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, bitchia_ca_crt, bitchia_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if bitchia_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return bitchia_init(root_path)


def bitchia_version_number() -> Tuple[str, str, str, str]:
...
def bitchia_minor_release_number():
    res = int(bitchia_version_number()[2])
    print(f"Install release number: {res}")
    return res


def bitchia_full_version_str() -> str:
    major, minor, patch, dev = bitchia_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def bitchia_init(root_path: Path):
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            f"warning, your CHIA_ROOT is set to {os.environ['CHIA_ROOT']}. "
            f"Please unset the environment variable and run bitchia init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"BitChia directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run bitchia init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'bitchia keys show --show-mnemonic-seed'")
