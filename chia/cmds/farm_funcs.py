...
from bitchia.rpc.farmer_rpc_client import FarmerRpcClient
from bitchia.rpc.full_node_rpc_client import FullNodeRpcClient
from bitchia.rpc.harvester_rpc_client import HarvesterRpcClient
...
from bitchia.util.misc import format_bytes
from bitchia.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total bitchia farmed: {amounts['farmed_amount'] / units['bitchia']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['bitchia']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['bitchia']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'bitchia start wallet' and 'bitchia wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'bitchia wallet show'")
    else:
        print("Note: log into your key using 'bitchia wallet show' to see rewards for each key")
